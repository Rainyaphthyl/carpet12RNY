...
public class SpawningCalculator {
    ...
    private final Set<BlockPos> startingPointSet = new HashSet<>();
    ...
    public synchronized void initCheckPossibleTargets() {
        ...
        for (int x = posCornerMin.getX(); x <= posCornerMax.getX(); ++x) {
            for (int z = posCornerMin.getZ(); z <= posCornerMax.getZ(); ++z) {
                int height = Math.min(getPossibleHeight(x, z), posCornerMax.getY() + 1);
                for (int y = posCornerMin.getY(); y < height; ++y) {
                    BlockPos posTarget = new BlockPos(x, y, z);
                    if (isSpawningPossible(posTarget, null)) {
                        possibleTargetSet.add(posTarget);
                        for (int dx = -20; dx <= 20; ++dx) {
                            for (int dz = -20; dz <= 20; ++dz) {
                                startingPointSet.add(posTarget);
                            }
                        }
                    }
                }
    ...
    /**
     * @param blockX X coordinate of the target block
     * @param blockZ Z coordinate of the target block
     * @return The maximum height around the target block, in an {@code 41*41} area
     */
    public synchronized int getPossibleHeight(int blockX, int blockZ) {
        long index = ChunkPos.asLong(blockX, blockZ);
        if (realHeightMap.containsKey(index)) {
            return realHeightMap.get(index);
        }
        initSetTargetRange();
        int height = 0;
        for (int dx = -20; dx <= 20; ++dx) {
            for (int dz = -20; dz <= 20; ++dz) {
                int temp = access.getSpawningColumnHeight(blockX + dx, blockZ + dz);
                if (temp > height) {
                    height = temp;
                }
            }
