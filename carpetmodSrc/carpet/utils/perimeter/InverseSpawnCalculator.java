...
public class InverseSpawnCalculator {
    ...
    private InverseSpawnCalculator(@Nonnull WorldServer world, Vec3d posPeriCenter, BlockPos posCorner1, BlockPos posCorner2) throws NullPointerException {
    ...
    public static InverseSpawnCalculator createInstance(WorldServer world, Vec3d posPeriCenter) {
        try {
            return new InverseSpawnCalculator(world, posPeriCenter, null, null);
    ...
    @Nullable
    @ParametersAreNonnullByDefault
    public static InverseSpawnCalculator createInstance(WorldServer world, BlockPos posTarget) {
        try {
            return new InverseSpawnCalculator(world, null, posTarget, posTarget);
        } catch (NullPointerException e) {
            return null;
        }
    }
    ...
    @Nullable
    @ParametersAreNonnullByDefault
    public static InverseSpawnCalculator createInstance(WorldServer world, BlockPos posCorner1, BlockPos posCorner2) {
        try {
            return new InverseSpawnCalculator(world, null, posCorner1, posCorner2);
        } catch (NullPointerException e) {
            return null;
        }
    }
        ...
        if (world instanceof WorldServer) {
            ...
            HttpUtil.DOWNLOADER_EXECUTOR.submit(() -> {
                try {
                    ...
                    InverseSpawnCalculator calculator = null;
                    switch (mode) {
                        case BLOCK:
                    ...
                    Messenger.m(sender, "w Potential positions: " + calculator.possibleTargetSet.size());
                    result.forEach((mobClass, value) -> {
                        ...
                        Messenger.m(sender, styles[0] + "  " + name, "g : chance: ",
                                styles[0] + " " + String.format("%f", ratePerRound), "^" + styles[1] + " " + ratePerRound,
                                "g , rate: ", "c " + String.format("%.1f", ratePerHour), "^q " + ratePerHour, "w /h");
                    });
                } catch (Throwable e) {
    ...
    public double getSpawningStepRate(int mobId, @Nonnull BlockPos posCurr, int rounds, int groupSize) {
        ...
        Double value = spawningRateCache.get(key);
        if (value != null) {
            ++cachingCount;
            return value;
        }
        double rateCurr;
        if (rounds == groupSize) {
            rateCurr = 3.0 * getChancePosInChunk(posCurr) * getChanceGroupSize(groupSize);
    ...
    public double getChanceGroupSize(int groupSize) {
        if (groupSize >= MIN_GROUP_NUM && groupSize <= MAX_GROUP_NUM) {
            return 1.0 / (MAX_GROUP_NUM - MIN_GROUP_NUM + 1);
        } else {
            return 0.0;
        }
    }
    ...
    public double getChanceMobAtBiome(int mobId, @Nonnull BlockPos posTarget) {
        Map<BlockPos, Double> blockMap = biomeChanceCache.computeIfAbsent(mobId, lambdaMobId ->
